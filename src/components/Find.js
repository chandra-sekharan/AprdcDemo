...
const Find = ()=>{
   ...
   const [load , setLoad] = useState('');
    ...
    const searchdata =()=>{
      setLoad(true)
      fetch(`http://localhost:3001/details/${search}`).then((Response)=>Response.json()).then((data)=>setDetail(data),
      setLoad(false)
      )
    ...
    return(
        ...
        <div className='findAPRDC'>
            <center><img src={detail['image']} alt="" /></center>
            <center><h2>{detail.fname}</h2><br></br></center>
            <p>Branch: <b>{detail['branch']}</b></p><br></br>
            <p>Student ID: <b>{detail['hallnumber']}</b></p><br></br>
            <span>Email: <b>{detail['email']}</b></span><br></br><br></br>
            <p>Phone No: <b>{detail['mnumber']}</b></p>
